...
void pasarg(int argc, char * argv[], Server_Context * context)
{
    ...
    {
        if(strncmp(argv[Loopi], "-s", 2) == 0)
        {
            context->bsavefile = 1;
        }
        else if(strncmp(argv[Loopi], "-h", 2) == 0)
        {
            showhelp(argv[0]);
            exit(0);
        }
        ...
        {
            strncpy(context->basename, argv[Loopi], sizeof(context->basename));
            strncpy(context->outfilename, argv[Loopi], sizeof(context->basename));
        }
    }
}
...
int main(int argc, char * argv[])
{
    ...
    if (read_at24c02b(225) == 11)
    {
        ...
        if (card_tlimit == 65535)
            card_tlimit = 30;
        /*set open mode on boot*/
        if (read_at24c02b(239) == 1)
            device_mode = 1;
        else
            device_mode = 0;
