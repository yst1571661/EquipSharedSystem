...
typedef struct connectSock
{
    ...
    /*互斥锁 保护当前socket用户接收缓冲区*/
    pthread_mutex_t lockBuffIn;
    /*互斥锁 保护当前socket用户发送缓冲区*/
    ...
    *length = 0;
    ...
    case 0x07:
            ...
            *length = 5;
            ...
            for (temp = 3; temp < dataLenth;)
            {
                ...
                {
                    /*判断是否为操作员，注意服务器同步超级用户的时候是'S'+卡号*/
                    /*将需要添加用户卡号放在TempBuffer中*/
    ...
    case 0x11:
            ...
            *length = 0;
            ...
            /*将收到服务器发送的卡号与发送过去的卡号进行比较，相同则进入判断*/
            {
                /*判断服务器状态是否正确*/
                if (dataBuffer[9] == 0x00)
                    /*正确则应答1*/
                    ...
                    /*出错应答0*/
                    card_ack = 0;
                /*收到应答则对所有等待参数条件cardquery_cond的线程接触阻塞*/
    ...
    case 0x13:
            ...
            /*得到数据表ID字节数,现在dataBuffer[temp]应指向'_'*/
            /*应答字长为应答状态+0x02+0x13+数据表ID,数据ID长度为temp-1个字节*/
            *length = 35;
...
int _ConnLoop()
{
    ...
    while(1)                                      		//网络检测
    {
        ...
        for (Loopi=0; Loopi<MAX_LINK_SOCK; Loopi++)           //在建立的socket集中查找请求读写的socket
        {
#ifdef DEBUG
            DebugPrintf("\n----_ConnLoop--check to read slot: %d", Loopi);
#endif
            if (FD_ISSET(sttConnSock[Loopi].fdSock, &carchExEvents))		//检测carchExEvents集是否变化
            {
                if (recv(sttConnSock[Loopi].fdSock, &oobytes, 1, MSG_OOB) < 0)
                {
                    //DebugPrintf("\n-----MSG_OOB < 0-----\n");
                }
                sttConnSock[Loopi].noProbes = 0;      //心搏函数监测值归零
            }
            else
            {
                if (sttConnSock[Loopi].fdSock > 0)
                {
                    FD_SET(sttConnSock[Loopi].fdSock, &exEvents);               //恢复监听该socket连接带外数据
                }
            }

            if (FD_ISSET(sttConnSock[Loopi].fdSock, &catchRdEvents))  //接收数据和客户关闭连接时都进入此处
            {
                DebugPrintf("\n-----if (FD_ISSET(sttConnSock[%d].fdSock, &catchRdEvents))-----",Loopi);
                sttConnSock[Loopi].noProbes = 0;      //心搏函数监测值归零
                fdClose = DispatchPacket(&sttConnSock[Loopi]);      //通信流读到用户缓冲区
                ...
                if (sttConnSock[Loopi].isPid == 0)          //线程未创建
                {
...
#endif
                        FD_SET(sttConnSock[Loopi].fdSock, &rdEvents);    //恢复可用socket监听
                    }
                }
            }
#ifdef DEBUG
...
static  void sync_card()
{
    ...
    while (time_count < time_wait) {
        ...
        if (cur_cardsnr > 1 && cur_cardsnr != -1 && cur_cardsnr != -2) {
            ...
            card_sendcount = 0;
            time_count = 0;
            /*让终端一直处于发送状态，直到服务器有应答*/
            do {
                /*发送卡号，返回0表示服务器有响应，返回-1表示超时*/
                if (0 == send_check(card)) {
                    /*判断服务器状态，card_ack为1:服务器状态正确，为0:服务器出错(出错需要服务器再次发送命令，让终端进入状态)*/
                    if (!card_ack)
                        return;
                    /*响两声表示服务器读到卡号，状态正确*/
                    card_beep(50);
...
/******************超级卡********************************************/
                    ...
                    if (gdbm_exists(gdbm_user, key) != 0)
                    {
                        ...
                        PrintScreen("\n----obtain a super card super_card = %s, cardrecordwr = %s-------", super_card, cardrecordwr);
#endif
                        /*判断首位是否为0检测当前是否有人使用设备，有人使用进入if语句内让他下机，没人使用则跳过*/
                        if (0 != cur_card[0])
                        {
                            /*第一次读到物理卡信息需延时30s,置延时标记位为1*/
                            Led_delay = 1;
                            /*将系统时间写入cardrecordwr*/
...
/***************非机组卡**********************************************/
                    ...
                    if (gdbm_exists(gdbm_user, key) != 0)
                    {
                        ...
                        else
                        {
                            /*当前设备有人使用*/
                            sprintf(cardrecordwr, "%.14s_%.14s_", card_time, read_sys_Time);
                            /*将当前使用卡添加到记录中，因为要返回与上机者相同的卡号*/
                            strncat(cardrecordwr,cur_card,8);
                        }
                        /*判断是否有人上机*/
                        if(cur_card[0]!=0){
                                /*对于非机组用户来说，如果正在上机的不是自己的话则跳出*/
...
/***************机组卡**********************************************/
                    ...
                    pthread_mutex_lock(&cardfile_lock);
                    /*返回1则用户存在，可以使用(user_temp因为没有加'S'，'N'，所以即使他为超级用户非机组用户不能检测到)*/
                    if (gdbm_exists(gdbm_user, key) != 0)
                    {
                        DebugPrintf("\n--------got an admin card:%s-------------------",this_card);
                        PrintScreen("\n--------got an admin card:%s-------------------",this_card);
                        pthread_mutex_unlock(&cardfile_lock);
                        /*表示现在电源状态需要变化(可以是上机也可以是下机)*/
                        Led_delay = 1;
                    }
                    else
                    { 											//返回0则用户不在
...
int WorkThreadCreate(ptexec threadexec, int prio) // 创建线程
{
    ...
    /*SCHED_FIFO --先进先出；SCHED_RR--轮转法；SCHED_OTHER--其他*/
